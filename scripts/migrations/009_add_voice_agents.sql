-- Per-account voice agent: which agent answers this account's calls, and
-- any audio/provider override. Read by the portal's Voice screen; written
-- only through the portal API.
...
-- write path treats a missing row as NOT entitled (fail closed), so a
